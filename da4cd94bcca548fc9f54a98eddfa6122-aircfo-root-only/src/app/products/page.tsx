...
export default function ProductsPage() {
  ...
  const noProductsText = locale === 'vi' ? 'Chưa có sản phẩm nào được tìm thấy' : 'No products found';

  return (
    <div className="container mx-auto px-4 py-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div>
          <h1 className="text-4xl font-bold mb-2">{locale === 'vi' ? 'Sản phẩm Affiliate' : locale === 'de' ? 'Affiliate-Produkte' : 'Affiliate Product Reviews'}</h1>
          <p className="text-muted-foreground">{locale === 'vi' ? 'Các thiết bị và sản phẩm tốt nhất cho người làm affiliate.' : 'The best gear and products for affiliate marketers.'}</p>
        </div>
      ...
      {productReviews.length > 0 ? (
            ...
            <Card key={review.id} className="flex flex-col h-full overflow-hidden hover:shadow-md transition-shadow border-2 hover:border-primary/20">
              ...
              <CardFooter className="pt-0 border-t mt-auto py-4">
                <Button variant="ghost" className="w-full text-primary" asChild>
                  <Link href={`/reviews/${review.slug}`}>{locale === 'vi' ? 'Đọc bài review' : 'Read Full Review'}</Link>
                </Button>
              </CardFooter>
            </Card>
          ))}
        ...
        <div className="flex flex-col items-center justify-center py-24 bg-muted/10 rounded-3xl border-2 border-dashed">
            ...
            <p className="text-xl font-medium text-muted-foreground italic">{noProductsText}</p>
        </div>
      )}
    </div>
  );
